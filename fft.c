...
static dcomplex* _execute_dft(dcomplex* data, unsigned int N, bool is_forward) {
	...
	for (unsigned k = 0; k < N; k++) {
		for (unsigned n = 0; n < N; n++) {
			*(r + k) = cadd(*(r + k),
				cmul(*(data + n), complex_exp(cbuild(0., -2. * PI * k * n * b / N)))
			);
		}
	}
...
static dcomplex* _raw_radix2fft(dcomplex* data, unsigned N, bool is_forward, bool keep_input) {
	...
	for (unsigned i = 2; i <= N; i <<= 1) {
		dcomplex step = complex_exp(cbuild(0., -2. * PI * b / i));
...
static dcomplex* _raw_raderfft(dcomplex* data, unsigned N, bool is_forward, bool keep_input) {
	...
	*b = complex_exp(cbuild(0., -2 * PI * k / N));
	for (unsigned q = 1; q < N - 1; q++) {
		*(a + q + M - N + 1) = *(data + expModuloN(g, q, N));
		*(b + q) = complex_exp(cbuild(0., -2 * PI * *(g_mod_minus_q + q - 1) * k / N));
		// pow(g, -2) (mod N)
...
static dcomplex* _raw_bluesteinfft(dcomplex* data, unsigned N, bool is_forward, bool keep_input) {
	...
	for (unsigned n = 0; n < N; n++) {
		...
		*(a + n) = cmul(*(data + n), complex_exp(cbuild(0, -twiddle)));
		*(b + n) = complex_exp(cbuild(0, twiddle));
	}
	if (!keep_input) free(data);
	for (unsigned n = M - N + 1; n < M; n++) {
		unsigned p = M - n;
		*(b + n) = complex_exp(cbuild(0, common * p * p));
	}
	...
	for (unsigned n = 0; n < N; n++) {
		*(result + n) = _Cmulcr(cmul(*(result + n), complex_exp(cbuild(0, -common * n * n))), 1. / M);
	}
